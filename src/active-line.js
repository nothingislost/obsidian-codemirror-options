...
(function(mod) {
    ...
    function updateActiveLines(cm, ranges) {
      ...
      for (var i = 0; i < ranges.length; i++) {
        ...
        if (typeof option == "object" && option.nonEmpty ? false : !range.empty())
          continue
        // nothingislost: support forwards and backwards multi line selections
        if (range.head.line > range.anchor.line) {
            var start = range.anchor.line, end = range.head.line
        } else {
            var start = range.head.line, end = range.anchor.line
        }
